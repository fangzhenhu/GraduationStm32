...
#define MEM1_BLOCK_SIZE			32  	  						//内存块大小为32字节 ，对内存块的 大小已经确定。
#define MEM1_MAX_SIZE			40*1024  						//最大管理内存 45K	 //最大内存应该是 40k
#define MEM1_ALLOC_TABLE_SIZE	MEM1_MAX_SIZE/MEM1_BLOCK_SIZE 	//内存表大小  这个数1280 表示的应该是 个数， 不是 字节数。 
...
struct _m_mallco_dev
{
	void (*init)(u8);					//初始化
	u8 (*perused)(u8);		  	    	//内存使用率
	u8 	*membase[2];					//内存池 管理2个区域的内存  这里是 指针数组。
	u16 *memmap[2]; 					//内存管理状态表 ，在脑海里记住 memmap 就是一个内存池的管理表格。
