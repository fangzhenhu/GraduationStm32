...
SD_Error SD_Init(void)
{
  ...
  GPIO_ResetBits(GPIOB, GPIO_Pin_5) ;	 // 为什么 要初始 化 pb5 呢；
  }
  
  /* Configure SDIO interface GPIO */
  GPIO_Configuration();		// 已看；
...
SD_Error SD_PowerON(void)
{
  ...
  SDIO_SetPowerState(SDIO_PowerState_ON);	//	这个是系统命令；
  ...
  errorstatus = CmdError();	  //发送命令0；	   这个只是检测 Cmd 是否发送出去；
  ...
  errorstatus = CmdResp7Error();   //	  根据 SDIO->sta 来判断是否 有响应；
  ...
  else
  {
    ...
    SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
    SDIO_SendCommand(&SDIO_CmdInitStructure);
    errorstatus = CmdResp1Error(SDIO_APP_CMD); //SDIO_APP_CMD 表示 CMD55;
  }
  /* CMD55 */	  //这里就再发送一次；
  SDIO_CmdInitStructure.SDIO_Argument = 0x00;
  SDIO_CmdInitStructure.SDIO_CmdIndex = SDIO_APP_CMD;
  ...
  if (errorstatus == SD_OK)
  {
    ...
    while ((!validvoltage) && (count < SD_MAX_VOLT_TRIAL))  //这里 竟然是 循环发送cmd55；，然后 再发送 ACMD41；
    {

      /* SEND CMD55 APP_CMD with RCA as 0 */	//==========这里的 RCA 表示 相对卡地址（Relative card address）寄存器:本地系统 中卡的地址 （是由 主机确定的）；
      SDIO_CmdInitStructure.SDIO_Argument = 0x00;
      SDIO_CmdInitStructure.SDIO_CmdIndex = SDIO_APP_CMD;
      SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
      SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
      ...
      SDIO_CmdInitStructure.SDIO_Argument = SD_VOLTAGE_WINDOW_SD | SDType; //  ((u32)0x80100000) ， ((u32)0x00000000)这 两个参数表示 什么？
      SDIO_CmdInitStructure.SDIO_CmdIndex = SDIO_SD_APP_OP_COND; // 这个就是 41 ；
      SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
      SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
      SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
      SDIO_SendCommand(&SDIO_CmdInitStructure);

      errorstatus = CmdResp3Error();  //返回的是R3 包含 sd 卡的OCR寄存器的值
      if (errorstatus != SD_OK)
      {
        return(errorstatus);
      }

      response = SDIO_GetResponse(SDIO_RESP1);	 //SDIO_Response_Registers
...
SD_Error SD_GetCardInfo(SD_CardInfo *cardinfo)
{
  ...
  if ((CardType == SDIO_STD_CAPACITY_SD_CARD_V1_1) || (CardType == SDIO_STD_CAPACITY_SD_CARD_V2_0))
  {
	...
	printf("\n\r the card type：SDIO_STD_CAPACITY_SD_CARD_V1_1或SDIO_STD_CAPACITY_SD_CARD_V2_0");
	printf("\n\r ShenZhouSD CardBlockSize %d SD card capacity： %d M", cardinfo->CardBlockSize, cardinfo->CardCapacity / 1024 / 1024);
  }
  else if (CardType == SDIO_HIGH_CAPACITY_SD_CARD)
  {
	...
	printf("\n\r the card type：SDIO_HIGH_CAPACITY_SD_CARD");
	printf("\n\r ShenZhouSD CardBlockSize %d CardCapacity %d M", cardinfo->CardBlockSize, cardinfo->CardCapacity / 1024 / 1024);   
  }
...
static void GPIO_Configuration(void)
{
  ...
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC | RCC_APB2Periph_GPIOD, ENABLE); //这句话就说明 是采用 SDIO 方式来驱动；
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;	 //这个是 CMD  命令线；
