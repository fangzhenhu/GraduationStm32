...
{  
	u8 byte=0;   
	SPI_FLASH_CS=0;                            //使能器件  ,从这里可以看出 低电平 使能。 
...
{   
	SPI_FLASH_CS=0;                            //使能器件   
	SPIx_ReadWriteByte(W25X_WriteStatusReg);   //发送写状态寄存器命令    
...
/************************************************************************/
/* 获取器件地址  因为SPI总线上可以挂很多的器件，所以首先要获得器件的地址。
获得器件地址的步骤是：拉低CS——>向器件发送获得地址（ID)的命令——>连续发送三个任意数据——>在发送第四个任意数据时在DO口上读出器件地址。
注意，发送完读地址后，从机 会将数据自动传给主机的SPI数据寄存器中，用户只要到该寄存器中取数就可以了。 
好的，既然用到写命令那么我们写来写这两个最基础的程序。                                                                     */
/************************************************************************/
//读取芯片ID W25X16的ID:0XEF14
u16 SPI_Flash_ReadID(void)
{
	//CS是片选信号，只有将该位拉低才能选中该芯片
	u16 Temp = 0;	  
	SPI_FLASH_CS=0;	  // 我可以 这样理解吗  当 spi_Flash_cs 为0  也就是当 cs  为低电平时 表示使能 Flash 吗；			    
	SPIx_ReadWriteByte(0x90);//发送读取ID命令	    
	SPIx_ReadWriteByte(0x00); 	    
	SPIx_ReadWriteByte(0x00); 	    
	SPIx_ReadWriteByte(0x00); 	 			   
	Temp|=SPIx_ReadWriteByte(0xFF)<<8;  // 表示 Flash 的 地址是 16 位的。
...
{ 
    ...
    SPIx_ReadWriteByte((u8)((ReadAddr)>>16));  //发送24bit地址  //从这里 可以看出 这种Flash 的 最大容量为 16M byte;
...
void SPI_Flash_Write_Page(u8* pBuffer,u32 WriteAddr,u16 NumByteToWrite)
{
    ...
    SPIx_ReadWriteByte((u8)WriteAddr);   // (u8)(writeAddr) 得到的是最低八位。
...
{ 
	...
	{	
		...
		for(i=0;i<secremain;i++)//校验数据 ，这里 secremain 可能=NumByteTowrite;
		{
			if(SPI_FLASH_BUF[secoff+i]!=0XFF)break;//需要擦除 ,原来如此 	  
		}
		//////////////////////////////////////////////////////////////////////////
		if(i<secremain)//需要擦除
		{
		...
		else // 在 这里说明 从writeAddr 一直到后面，都是没有使用过的。
			{
