...
SD_Error SD_Init(void)
{
  ...
  /* Enable the DMA2 Clock ,，这里面是 可以使用DMA2 的*/
...
SD_Error SD_PowerON(void)
{
  ...
  /* CMD0: GO_IDLE_STATE ，表示 输入CMD0 进入闲置状态-------------------------------------------------------*/
  /* No CMD response required ，向外设发送命令后，这里不需要 等待回复*/
  SDIO_CmdInitStructure.SDIO_Argument = 0x0;
  SDIO_CmdInitStructure.SDIO_CmdIndex = SDIO_GO_IDLE_STATE;//0
  ...
  errorstatus = CmdResp7Error();// 这个表示对于 cmd8 命令  外设返回的是 REsponse 7;

  if (errorstatus == SD_OK)// 如果返回值 为 SD_OK 则表示 检测到 Ver 2.0
  {
    CardType = SDIO_STD_CAPACITY_SD_CARD_V2_0; /* SD Card 2.0 */
    SDType = SD_HIGH_CAPACITY;
  }
  else // 如果没有检测的返回值，很可能是 ver 1.0 ,不过还需要 再用 CMD55 来表示。
  {
    /* CMD55 */
  ...
  /* CMD55 这个表示 刚才的 SD_OK 的时候 还需要用CMD 55 */
  //CMD55是一个特殊的指令，该指令告诉主机下一个command将会是一个应用指令（application command），
  //比如要发送ACMD41之前必须先发送CMD55。此处发送CMD55是为了检测卡的类型。
  //不过暂时没没看懂为什么是发送CMD55,因为关于CMD55的解释中并没有提到它能够检测卡类型，只说了它的应答类型是R1。
  //判断是SD卡或者是不支持的卡。
  SDIO_CmdInitStructure.SDIO_Argument = 0x00;
  SDIO_CmdInitStructure.SDIO_CmdIndex = SDIO_APP_CMD;
  SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
...
static void GPIO_Configuration(void)
{
  ...
  /* Configure PC.08, PC.09, PC.10, PC.11, PC.12 表示初始化 Data0, Data1, Data2, Data3, CLK ,, 这里还有一个 pc.6  表示NCD*/
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8 | GPIO_Pin_12| GPIO_Pin_9| GPIO_Pin_10 | GPIO_Pin_11 |GPIO_Pin_6;
